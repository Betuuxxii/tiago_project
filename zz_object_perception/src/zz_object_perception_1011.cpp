...
//euclidean cluster extraction 
  ...
  *cloud_filtered = *cloud_f;
  ...
  for (int i=0; i<cluster_indices.size(); ++i) {
    ...
    ros::Publisher pub_po = nh.advertise<geometry_msgs::PoseStamped> ("/cluster_" + boost::lexical_cast<std::string>(i) + "/pose", 1);
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
    ...
    for (std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); pit++)
      ...
      geometry_msgs::PoseStamped cluster_pose_temp;
      cluster_pose_temp.header.frame_id = "xtion_optical_frame";
      cluster_pose_temp.header.stamp = ros::Time::now();
      cluster_pose_temp.pose.position.x = centroid[0];
      cluster_pose_temp.pose.position.y = centroid[1];
      cluster_pose_temp.pose.position.z = centroid[2];
      //drawrviz(cluster_pose_temp);
...
//Remove the planar inliers, extract the rest
  ...
  extract.filter(*env_cloud);
    pcl::toROSMsg(*env_cloud, *env_objects_cloud);
